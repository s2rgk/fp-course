...
string lst = sequenceParser $ is <$> lst
...
noneof lst = satisfy (`notElem` lst)
...
between open close body = open *> body <* close
...
-- True
hex :: Parser Char
hex = do str <- replicateA 4 (satisfy isHexDigit)
         case readHex str of
           Full i -> return $ chr i
           Empty  -> constantParser (UnexpectedString str)
...
sepby1 p sep = lift2 (:.) p (list $ sep *> p)
...
satisfyAll lst = satisfy $ and . sequence lst
...
satisfyAny lst = satisfy $ or . sequence lst
