...
map ::
  ...
  -> List a
  -> List b
map f = foldRight (\a lst -> f a :. lst) Nil
...
flatten ::
  List (List a)
  -> List a
flatten = foldRight (\xs lst -> xs ++ lst) Nil
...
flatMap ::
  ...
  -> List a
  -> List b
flatMap f = foldRight (\x xs -> f x ++ xs) Nil
...
find ::
  ...
  -> List a
  -> Optional a
find f = foldRight (\a opt -> if f a then Full a else opt) Empty
