...
instance Monad Parser where
  ...
  f =<< (P p) = P $ \input -> onResult (p input) $ \input' a -> parse (f a) input'
...
satisfy f = (\c -> if f c then valueParser c else unexpectedCharParser c) =<< character
...
firstNameParser = do first <- upper
                     rest  <- list lower
                     return $ first :. rest
...
surnameParser = do first <- upper
                   mid   <- thisMany 5 lower
                   rest  <- list lower
...
phoneParser = do firstDigit <- digit
